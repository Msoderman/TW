...
function overView(){
	...
	for(var i=0;i<n;i++){
		...
		if(k.match(/Assistans/i)){
			...
			if (hasArchers) {archer=parseInt(eleTrs[i+1].cells[getHeader('archer')].innerHTML);}
			scout=parseInt(eleTrs[i+1].cells[getHeader('spy')].innerHTML);
			lc=parseInt(eleTrs[i+1].cells[getHeader('light')].innerHTML);
			if (hasArchers) {marcher=parseInt(eleTrs[i+1].cells[getHeader('marcher')].innerHTML);}
			...
			if (hasPaladin) {pala=parseInt(eleTrs[i+1].cells[getHeader('knight')].innerHTML);}
			...
			if (findMe(coord) == -1 || snipe == "y") {
				...
				villages[p].push(axe);
				if (hasArchers) {villages[p].push(archer);}
				else { villages[p].push(0);}
				villages[p].push(scout);
				villages[p].push(lc);
				if (hasArchers) {villages[p].push(marcher);}
				else { villages[p].push(0);}
				villages[p].push(hc);
				villages[p].push(ram);
				villages[p].push(cat);
				if (hasPaladin) {villages[p].push(pala);}
				else { villages[p].push(0);}
				...
				villages[p][4] += sword;
				villages[p][5] += axe;
				if (hasArchers) {villages[p][6] += archer;}
				villages[p][7] += scout;
				villages[p][8] += lc;
				if (hasArchers) {villages[p][9] += marcher;}
				villages[p][10] += hc;
				villages[p][11] += ram;
				villages[p][12] += cat;
				if (hasPaladin) {villages[p][13] += pala;}
				villages[p][14] += noble;
			}		
		}
...
for (var q = 1; q < villages.length; q++) {
	...
	if(villages[q][6] !== 0) {docSource += "[unit]archer[/unit]" + villages[q][6] + " ";}
	if(villages[q][7] !== 0) {docSource += "[unit]spy[/unit]" + villages[q][7] + " ";}
	if(villages[q][8] !== 0) {docSource += "[unit]light[/unit]" + villages[q][8] + " ";}
	if(villages[q][9] !== 0) {docSource += "[unit]marcher[/unit]" + villages[q][9] + " ";}
	if(villages[q][10] !== 0) {docSource += "[unit]heavy[/unit]" + villages[q][10] + " ";}
	if(villages[q][11] !== 0) {docSource += "[unit]ram[/unit]" + villages[q][11] + " ";}
	if(villages[q][12] !== 0) {docSource += "[unit]catapult[/unit]" + villages[q][12] + " ";}
	if(villages[q][13] !== 0) {docSource += "[unit]knight[/unit]" + villages[q][13] + " ";}
	if(villages[q][14] !== 0) {docSource += "[unit]snob[/unit]" + villages[q][14] + "";}
	docSource += " = ";
	var farm = parseInt(parseInt(villages[q][3])*spearFarm+parseInt(villages[q][4])*swordFarm+parseInt(villages[q][5])*axeFarm+parseInt(villages[q][6])*archerFarm+parseInt(villages[q][7])*spyFarm+parseInt(villages[q][8])*lightFarm+parseInt(villages[q][9])*marcherFarm+parseInt(villages[q][10])*heavyFarm+parseInt(villages[q][11])*ramFarm+parseInt(villages[q][12])*catapultFarm);
